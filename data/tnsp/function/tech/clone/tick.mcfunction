...
# 分身をジャンプさせる
execute if score @s kt.detect_jump matches 1.. as @e[tag=kt.clone,tag=kt.this] if entity @s[nbt={OnGround:1b}] run data modify entity @s Motion[1] set from entity @p[tag=kt.clone_tmp] Motion[1]
execute if score @s kt.detect_jump matches 1.. run scoreboard players set @s kt.detect_jump 0

# 分身のタイマーを増加させ、一定数以上で殺す
scoreboard players add @e[tag=kt.clone,tag=kt.this] kt.timer 1
execute if entity @e[scores={kt.timer=100..},tag=kt.clone,tag=kt.this] run function tnsp:tech/clone/kill
