...
execute unless entity @e[tag=kt.this] run return run function tnsp:clone/kill
execute as @n[tag=kt.this] at @s run function tnsp:tech/clone/walk
tag @e remove kt.this

# 分身をジャンプさせる
execute if score @s kt.detect_jump matches 1.. as @e[tag=kt.clone] if score @s kt.uuid = @p[tag=kt.clone_tmp] kt.uuid run data modify entity @s Motion[1] set from entity @p[tag=kt.clone_tmp] Motion[1]
