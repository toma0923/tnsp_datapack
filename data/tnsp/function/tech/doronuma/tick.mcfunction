...
# 一定時間で消える
scoreboard players add @e[tag=kt.doronuma] kt.timer 1
execute as @e[tag=kt.doronuma,scores={kt.timer=600..}] at @s run summon area_effect_cloud ~ ~ ~ {custom_particle:{type:"smoke"},Radius:1f,Duration:25,Age:20}
execute as @e[tag=kt.doronuma] if score @s kt.timer matches 600.. at @s run kill @s

# 泥沼内の敵に鈍足付与
execute as @a run attribute @s jump_strength base set 0.42
execute as @e[tag=kt.doronuma] at @s if entity @p[distance=..1] run function tnsp:tech/doronuma/slow